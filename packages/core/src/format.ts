// Token + amount formatting shared by the browser app and the `dshield` CLI, so
// both agree on decimals and how stroop <-> USDC conversions round.
 ...
 * a note's exact committed value: the amount goes into the commitment hash and
 * into the on-chain transfer, and those two must agree to the stroop. A float
 * round-trip of something like "1234567.8912345" lands a unit or two off and the
 * note becomes unspendable. Anything finer than {@link TOKEN_DECIMALS} is
 * truncated, not rounded, so a deposit never asks for more than typed.
 */
 ...
 * e.g. "137.42 USDC", "0.5 USDC", "100 USDC". Never rounds the value away.
 */
